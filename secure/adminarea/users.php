                    ...
                    <table class="center">
                        ...
                        ?>
                            <tr class="adminpagedata adminpagetableentry">
                                <td><a href="../../studentarea/statistics/profile/<?= $row['username'] ?>" target="_blank"><img draggable="false" class="mini-pfp-35" src="<?= getProfilePicture($row['id']) ?>" <?= onerror("https://cdn.surge-networks.co.uk/assets/content/uploads/noimg.png") ?>></a></td>
                                <td><?= $row['username'] ?></td>
                                <td><?= $row['firstname'] ?></td>
                                <td><?= $row['lastname'] ?></td>
                                <td style="max-width: 10px; overflow: hidden; text-overflow: ellipsis;" title="<?= $row['email'] ?>"><?= $row['email'] ?></td>
                                <td><?= determineAccountType($row['account_type']) ?></td>
                                <td><?= $row['class_id'] ?></td>
                                <td><?= $row['id'] ?></td>
                                <td title="<?= convertDate($row['last_seen'], "F j, Y, g:i a") ?>"><?= convertDate($row['last_seen'], "F j, g:i a") ?></td>
                                <!-- <td>
                                    <?php
                                    ...
                                    ?>
                                        <a href2="modules/delete_account?id=<?= $row['id'] ?>" href="#delete-account"><i class="fad fa-trash"></i> Delete</a>
                                    <?php
                                    }
                                    ?>
                                </td> -->
